...
const knownPrefix = "<hbe-prefix></hbe-prefix>";
  ...
  function hbeLoader() {
    ...
    if (passwordInput) {
      // 监听多种事件类型以确保兼容性
      ['keydown', 'keypress', 'input'].forEach(eventType => {
        passwordInput.addEventListener(eventType, function(event) {
          // 检测空格键
      ...
      submitButton.className = 'hbe-submit-button';
      submitButton.style.cssText = 'margin-top: 10px; padding: 8px 15px; z-index: 1000; position: relative;';
      
      // 将按钮插入到密码框后
      passwordInput.parentNode.insertBefore(submitButton, passwordInput.nextSibling);
      ...
      submitButton.addEventListener('click', function(e) {
        ...
        setTimeout(async function() {
          try {
            ...
            console.log("尝试验证密码：", password.replace(/./g, '*')); // 安全记录
